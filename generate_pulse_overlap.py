...
profiles = {
    "airspymini": {
        "filename": "raw_iq_airspymini_3000000.bin",
        "sample_rate": 50000
    },
    "airspyhf": {
...
class generate_pulse_overlap:
    def __init__(self):
        ...
        self.first_pulse_idx = 1
        self.last_pulse_idx = 2
        self.pulse_overlap = 0.15
        ...
        comebined_iq, pulse_1_iq, pulse_2_iq  = self.overlap_pulses(pulse_times[self.first_pulse_idx], pulse_times[self.last_pulse_idx], overlap = self.pulse_overlap)

        self.plot_pulse(comebined_iq, pulse_1_iq, pulse_2_iq)


    def plot_pulse(self, comebined_iq, pulse_1_iq, pulse_2_iq):
        ...
        # Magnitude
        times = np.arange(len(comebined_iq)) / self.sample_rate
        axes[0,0].plot(times, 20 * np.log10(np.abs(pulse_1_iq) + 1e-12), color = "red")
        axes[1,0].plot(times, 20 * np.log10(np.abs(comebined_iq) + 1e-12))
        ...
        freqs_combined, fft_mag_combined = self.pulse_fft(comebined_iq, masked = self.fft_masked)
        ...
        print(f"Processing time for Gaussian fits: {(time2 - time1) / 1e3:.3f} ms")
        combined_curve = None
        for i, curve in enumerate(gaussians):
            curve_db = 20 * np.log10(curve + 1e-12)  # avoid log(0)
            
            print(f"Pulse {i+1}: peak at {centers[i]:.3f} Hz with magnitude {max(curve):.3f}")
            ...
            axes[i*2,1].axvline( x = centers[i] / 1e3, linestyle='--', color = ("darkred","darkgreen")[i] )
            axes[i*2,1].set_title(f"Pulse {i + 1} FFT ({np.abs(pulse_actual_peaks[i] - centers[i]):.1f} Hz diff)")
            if (i == 0):
    ...
    def overlap_pulses(self, pulse_1_times, pulse_2_times, overlap = 1):
        ...
        pulse_1_sample_start = self.sample_rate * pulse_1_times[0] - pulse_padding_samples
        pulse_1_sample_end = self.sample_rate * pulse_1_times[1] + pulse_padding_samples
        pulse_2_sample_start = self.sample_rate * pulse_2_times[0] - pulse_padding_samples
        pulse_2_sample_end = self.sample_rate * pulse_2_times[1] + pulse_padding_samples
        
        sample_shift = (1 - overlap) * (pulse_1_sample_end - pulse_1_sample_start - (pulse_padding_samples * 2))

        pulse_1_iq = self.raw_iq[int(pulse_1_sample_start):int(pulse_1_sample_end)]
        pulse_2_iq = self.raw_iq[int(pulse_2_sample_start) - int(sample_shift):int(pulse_2_sample_end)]


        if (len(pulse_1_iq) < len(pulse_2_iq)):
            pulse_1_iq = self.raw_iq[int(pulse_1_sample_start):int(pulse_1_sample_end) + (len(pulse_2_iq) - len(pulse_1_iq))]
        elif (len(pulse_1_iq) > len(pulse_2_iq)):
            pulse_2_iq = self.raw_iq[int(pulse_2_sample_start):int(pulse_2_sample_end) + (len(pulse_1_iq) - len(pulse_2_iq))]
    ...
    def get_pulse_times(self):
        ...
        for i in range(0, len(self.pulses)):
            ...
            pulse_dfreqs.append( float(pulse_dfreq_cubic) )
